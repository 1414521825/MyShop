...
> **背景适配**：Java 后端转鸿蒙客户端，优先补齐客户端线程模型、UI 状态、长列表性能、内存与帧率分析能力。
...
- **先深后广**：优先做深购物车、网络层、登录态、拼团倒计时、推荐流性能。偏运营展示类玩法通常由成熟跨端动态化体系承接，入职前不投入非主线原生能力死磕。
...
从 Mock 过渡到真实网络层思维，掌握统一请求状态、错误态、Token 管理、401 刷新队列、路由守卫，以及 ArkTS 单线程事件循环和 TaskPool 后台任务模型。
...
| `TextInput` | 手机号、验证码输入 |
| 路由守卫 | 登录前拦截需要权限的页面 |
| EventLoop | 理解 UI 主线程单线程事件循环，避免长任务阻塞渲染 |
| `@ohos.taskpool` / `TaskPool` | 大 JSON 解析、复杂数据归并、耗时计算切后台线程 |
...
鸿蒙客户端不是 Java 后端多线程模型。ArkUI 页面渲染、用户输入、状态更新都依赖 UI 主线程事件循环。网络请求本身可以异步，但**响应后的大体积 JSON 解析、字段清洗、排序归并、分页去重不能长期占用 UI 主线程**。

硬性要求：

- `HttpClient` 只负责请求、错误、状态码和小体积响应解析。
- 大体积推荐流、订单列表、购物车批量数据解析必须通过 `TaskPoolParser` 切到 TaskPool 线程。
- UI 主线程只接收已经解析好的 Model / DTO / ViewModel 输入，不做大循环解析。
- TaskPool 返回结果后再更新 `@Trace` 状态，避免后台线程直接操作 UI 状态。
- 每个 TaskPool 任务都要有失败兜底，失败后回到 `RequestState.error()`。
...
TaskPool
  -> 负责大 JSON 解析、复杂排序、去重、聚合等 CPU 任务
```
...
8. 敏感日志脱敏。
9. 实现 `TaskPoolParser`，把大体积 JSON 解析和推荐流数据去重切到后台线程。
10. 对比主线程解析和 TaskPool 解析的页面卡顿差异。
...
- [ ] 大体积 JSON 是否避免在 UI 主线程解析？
- [ ] TaskPool 任务失败是否能回到页面错误态？
...
掌握电商首页/推荐流高频能力：基于 API 12+ `Repeat` 的长列表渲染、分页、图片加载、列表复用、骨架屏、性能分析。阶段核心目标是把推荐流从“能展示”推进到“能稳定高帧率滑动”。
...
| --- | --- |
| `WaterFlow` | 双列瀑布流 |
| `Repeat` | API 12+ 长列表核心渲染组件，阶段 5 绝对核心 |
| `Refresh` | 下拉刷新 |
...
3. 图片加载支持占位、失败图、缓存策略。
4. 用 `Repeat` 重构推荐流列表，重点观察数据变更、复用、key 稳定性和状态串项。
5. `ForEach` 只作为小数据量或对照实验，不作为推荐流主实现。
6. 结合 `@ReusableV2`、`@Once`、稳定 key、图片占位尺寸，减少列表项重建和布局抖动。
7. Tab 切走时冻结不可见页面，回来时校验数据是否过期。
8. 输出一次优化前后页面打开耗时、内存、滚动 FPS 对比。

### 性能硬指标

- 推荐流连续滑动时 FPS 稳定在 **115+**，不能只看“体感不卡”。
- 快速滑动、分页追加、图片加载同时发生时，不出现明显掉帧。
- 单个列表项必须有稳定尺寸或可预测布局，禁止图片加载后大幅撑开导致重排。
- 大分页数据解析、去重、排序使用 `TaskPoolParser`，禁止在 UI 主线程做大循环。
- 用 `ProfilerAnalyzer` 记录优化前后帧率、CPU、内存、GC/对象分配趋势。

### V1/V2 对照思考

- `@Reusable` 和 `@ReusableV2` 生命周期差异是什么？
- `Repeat` 和 `ForEach` 在长列表 diff、复用、状态隔离上有什么差异？
...
- [ ] 图片失败是否有兜底？
- [ ] 推荐流滑动 FPS 是否稳定在 115+？
- [ ] 是否基于 `Repeat` 作为长列表主实现？
- [ ] 复用列表项是否出现状态串项？
- [ ] 分页追加时是否避免主线程大 JSON 解析？
...
| 阶段 4 | 倒计时精度、前后台恢复准确性 |
| 阶段 5 | `Repeat` 长列表、推荐流 FPS 115+、图片加载、分页耗时 |
...
| `TaskPoolParser` | 大体积 JSON、推荐流分页、订单列表等后台解析 | 阶段 2 起 |
...
| `ProfilerAnalyzer` | 帧率、内存、CPU、对象分配趋势分析方法 | 阶段 5 起 |
